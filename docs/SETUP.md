...
## 6단계 (선택). 갤러리에 동영상 올리기 — 약 1분

갤러리에 **동영상**도 올릴 수 있어요. 이걸 하기 전에는 사진만 올라가고,
버튼도 "사진 올리기"로만 보여요(앱이 알아서 사진 모드로 동작하니 고장나진 않아요).

1. Supabase 대시보드 → **SQL Editor**
2. 이 폴더의 `supabase/migration-media.sql` 내용을 붙여넣고 **Run**
3. 앱을 새로고침하면 버튼이 **"사진·동영상 올리기"**로 바뀌어요

- 이 SQL이 하는 일: `photos` 테이블에 칸 3개 추가 + 동영상 파일 보관함(`media`) 만들기 + 친구들이 올리고 지울 수 있는 권한.
- 동영상은 **한 개 50MB까지**, 전체 무료 용량은 **1GB**예요. 긴 영상은 잘라서 올려주세요.
- 사진과 달리 동영상 **파일 자체는 데이터베이스가 아니라 보관함**에 들어가요. 그래서 갤러리를 열 때 앱이 느려지지 않아요. 목록에는 첫 장면 미리보기 사진만 보이고, 눌러야 재생됩니다.

## 참고

- anon 키는 "링크 아는 사람은 쓸 수 있는 열쇠"예요. 친구들끼리만 링크를 공유하면 돼요. 위험한 관리자 키(`service_role`)는 절대 아무 데도 넣지 않아요.
- 무료 용량: Supabase 데이터베이스 500MB — 사진을 자동 압축(장당 약 0.1~0.2MB)하니 수천 장은 거뜬해요. 동영상은 별도 보관함 1GB를 씁니다(6단계).
