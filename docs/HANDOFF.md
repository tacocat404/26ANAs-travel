...
> 마지막 갱신: 2026-08-29. 새 세션의 Claude는 이 문서 + `CLAUDE.md` + `docs/PLAN.md` + `docs/DEVLOG.md`를 읽으면 맥락이 복원된다.

## 프로젝트가 뭔가
- **"언제갈까?"** - 친구들이 각자 "안 되는 날"을 색깔로 표시해 모두 갈 수 있는 여행 날짜를 찾는 웹앱.
- 위치: `C:\기미주\바이브 코딩 작품\일정캘린더` (2026-08-29에 `C:\기미주\일정캘린더`에서 이 안으로 옮겨짐). React + Vite. 운영자는 비개발자.
...
## 현재 실사용 데이터 (2026-08-29, 공유 DB)
- 멤버 11명, 여행 3개(🎂 유노 생일파티 / 🏖️ 26아박령 MT / 📖 LG aimers 일정 9/2), 사진·영상 13개, 공지·생일 라벨 있음. 친구들이 실제로 쓰고 있다.
- ⚠️ 실사용 중이므로 **테스트는 데모 모드(config 임시 변경)로 하고 반드시 원상 복구**할 것. 실 DB에 만든 테스트 데이터는 그 자리에서 지운다.

## 자주 나는 문제 (먼저 의심할 것)
- **사이트가 "불러오는 중…"에서 안 넘어간다** → 십중팔구 **Supabase 프로젝트가 일시정지(INACTIVE)**. 무료 플랜은 한동안 아무도 안 들어오면 자동으로 멈추고, 그러면 프로젝트 주소가 **DNS에서 사라진다**(`nslookup` 실패). Supabase 대시보드에서 **Restore** 누르면 2~3분 뒤 복구되고 **데이터는 그대로**다. (2026-08-29에 실제로 발생.)
  - 구분법: 화면은 뜨는데 데이터만 안 옴 = DB 문제 / 아예 흰 화면 = 빌드·경로 문제.
- **흰 화면(아무것도 안 그려짐)** → `vite.config.js`의 `base`와 실제 배포 주소가 안 맞는 경우. GitHub Pages는 `/26ANAs-travel/`, 최상위 주소면 `/`.
- **배포처는 GitHub Pages 하나만 쓴다** (2026-08-29에 Vercel 정리). main push = 자동 배포.
