...
import TripEditForm from './TripEditForm.jsx'
...
// 지도는 Leaflet까지 합쳐 무거워서, 2단계(세부 일정)를 열 때 따로 받아온다.
// 첫 화면 로딩에 지도 코드가 끼지 않아 앱이 훨씬 빨리 뜬다.
    ...
    <main className="page">
      ...
      {visited.has('plan') && (
        <div hidden={view !== 'plan'}>
          <Suspense fallback={<div className="map-lazy">지도를 준비하는 중…</div>}>
      ...
      <div className="trip-foot">
        {editing ? (
          <TripEditForm trip={trip} refresh={refresh} onClose={() => setEditing(false)} />
        ) : (
